...
class ConversationHistoryTabComponent extends React.Component<MyProps, MyState> {
    ...
    async componentDidUpdate(){
        if (this.state.phoneNumber != this.props.task?.attributes.from){
            const fetchConversationsRequest = await fetchConversationsByParticipant(this.props.task?.attributes.from, process.env.FETCH_CONVERSATIONS_AS_OLD_AS).then( (convos) =>{     
                            ...
                            return (    
                                <Disclosure key={conversation.conversationSid}>
                                    ...
                                    <DisclosureContent key={index}>
                                        <ConversationHistoryTranscriptComponent conversationSid={conversation.conversationSid} />
                                    </DisclosureContent>
                                </Disclosure>
                            ) 
                        })
